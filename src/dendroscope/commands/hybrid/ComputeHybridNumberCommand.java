/*
 *   ComputeHybridNumberCommand.java Copyright (C) 2020 Daniel H. Huson
 *
...
public class ComputeHybridNumberCommand extends ComputeHybridCommand {
    public void apply(NexusStreamParser np) throws Exception {
        np.matchIgnoreCase("compute hybridization-number method=ASCH2011");
        ...
        if (showDialog) {
            View view = new View(getViewer().getFrame(), controller, View.Computation.HYBRID_NUMBER);
        ...
        {
            controller.run(View.Computation.HYBRID_NUMBER, false);
            synchronized (this) {
                this.wait();
            }

            if (controller.getHybridNumber() != -1)
                new Message(getViewer().getFrame(), "Hybrid number: " + controller.getHybridNumber());
        }

    }

    public void actionPerformed(ActionEvent ev) {
        execute("compute hybridization-number method=ASCH2011 showdialog=true;");
    }

    public String getSyntax() {
        return "compute hybridization-number method=ASCH2011 [showdialog={false|true}] [numberOfThreads=number];";
    }
    ...
    public String getDescription() {
        return "Compute the hybridization number for two rooted binary trees using  using (Albrecht, Scornavacca, Cenci, and Huson, 2011)";
    }
    ...
    public String getName() {
        return "Hybridization Number (Binary Trees)...";
    }
