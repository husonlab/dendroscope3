/*
 * EmbeddingOptimizerNNet.java Copyright (C) 2023 Daniel H. Huson
 *
...
public class EmbeddingOptimizerNNet implements ILayoutOptimizer {
    ...
    public int apply(PhyloTree[] trees, ProgressListener progressListener, boolean shortestPath, boolean useFastAlignmentHeuristic) throws CanceledException {
		...
		for (PhyloTree tree : trees) {
			...
			var tax = TanglegramUtils.getTaxaForTanglegram(tree);
		...
		if (!useFastAlignmentHeuristic && trees.length == 2) {
			...
			while (swapTourNew < 5) {
                ...
                for (int s = 0; s < 2; s++) {
					...
					for (var a = 1; a < tempOrder[s].size(); a++) {
						...
						var lastOneInsOfThisTree = 0; //to avoid to mes up the ordering in the tree
							...
							if (stringToInsert.equalsIgnoreCase("rho****")) {
								...
								for (var p = lastOneInsOfThisTree; p < bestOrdForNow.size() + 1; p++) {  //to avoid to mes up the ordering in the tree
                                    ...
                                    if (taxaSetsThatAgree == alreadyInsertedSets.size()) {
										var value = TanglegramUtils.computeCrossingNum(copy, currOrderingListNew);
                                        if (value <= min) {
                                            min = value;
                                            bestOrdTemp.clear();
				...
				var score = TanglegramUtils.computeCrossingNum(newOrder[0], newOrder[1]);
				if (score == best)
					break;
			}
			...
			var finalScore = TanglegramUtils.computeCrossingNum(newOrder[0], newOrder[1]);   // the two orderings for Daniel
    ...
    public int[] computerCircularOrderingHardwiredMatrix(PhyloTree[] trees, Map<String, Integer> taxon2ID, Map<Integer, String> id2Taxon) {
        if (taxon2ID.size() > 2) {
			...
			if (trees.length == 2 && (taxaTrees[0].size() != taxon2ID.size() || taxaTrees[1].size() != taxon2ID.size())) {
				...
				var clustersAll = PhyloTreeUtils.collectAllHardwiredClusters(newTrees[0]);
                clustersAll.addAll(PhyloTreeUtils.collectAllHardwiredClusters(newTrees[1]));

				var sys = TanglegramUtils.getSplitSystem(clustersAll, taxon2ID);
...
// create a new distance matrix and update it for every split system induced by the given networks
				for (var tree : trees) {
					...
					var clusters = PhyloTreeUtils.collectAllHardwiredClusters(tree);
            ...
            try {
				var ordering = NeighborNetCycle.getNeighborNetOrdering(null, ntax, distMat);
                if (trees.length == 2) {
