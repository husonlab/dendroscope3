/**
 * EmbedderForOrderPrescribedNetwork.java 
...
public class EmbedderForOrderPrescribedNetwork {
    ...
    public static void apply(TreeViewer treeViewer, Map<Node, Float> node2pos) throws IOException {
        ...
        if (tree.getRoot() != null) {
            if (verbose)
                System.err.println("Original network: " + treeViewer.getPhyloTree().toString() + ";");
            ...
            if (verbose)
                System.err.println("Reordered network: " + treeViewer.getPhyloTree().toString() + ";");
        }
    }
    ...
    private static void processNestedSubTrees(NodeArray<Integer> node2SubTreeId, int numberOfSubTrees,
        ...
        for (int id = 1; id <= numberOfSubTrees; id++) {
            ...
            if (lsaParent != null) {
                ...
                BitSet spans = Cluster.intersection(before, after);
    ...
    public static Map<Node, Float> setupAlphabeticalOrdering(TreeViewer treeViewer) throws IOException {
        ...
        for (Node v = tree.getFirstNode(); v != null; v = v.getNext()) {
            ...
            if (label != null) {
                ...
                if (label.length() > 0) {
                    if (v.getOutDegree() > 0)
                        throw new IOException("Internal node has label: " + label);

                    if (taxLabels.contains(label))
                        throw new IOException("Multiple occurrence of label: " + label);
                    label2node.put(label, v);
                    taxLabels.add(label);
                }
            }
        }
        ...
        for (String label : taxLabels) {
            node2pos.put(label2node.get(label), (float) ++count);
        }
        return node2pos;
    }
