	...
	private void addSubtreesRec(HybridTree t, Vector<MyNode> nodes, Vector<HybridTree> subtrees, int index,
		...
		if (index < subtrees.size()) {
			for (MyNode sib : nodes) {
				...
				MyNode pCopy = sibCopy.getInEdges().next().getSource();
				tCopy.deleteEdge(sibCopy.getInEdges().next());
				...
				if ((subtrees.get(index).getNodes().size() == 1 && sibCopy.getOutDegree() != 0 && sibCopy.getInfo() != null)
					...
					// lca1.getInEdges().next().getSource() : lca1;
					// MyNode p1 = lca1.getInEdges().next().getSource();
					...
					b.and(inserted);
					while (b.equals(c1)) {
						lca1 = lca1.getInEdges().next().getSource();
