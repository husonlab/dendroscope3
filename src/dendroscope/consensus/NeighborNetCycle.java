...
public class NeighborNetCycle {
    ...
    static public int[] getNeighborNetOrdering(ProgressListener progressListener, Taxa taxa, SplitSystem splits) throws CanceledException {
        if (taxa.getBits().nextSetBit(taxa.ntax + 1) > 0) {
            System.err.println("getNeighborNetOrdering() failed: illegal taxa set");
            return null;
        }
        int ntax = taxa.size();
        int[] ordering = null;
        if (ntax < 4) {
            ...
            progressListener.setTasks("Using Neighbor-Net to compute cycle", "");
     ...
     */
    static public int[] getNeighborNetOrdering(ProgressListener progressListener, int ntax, double[][] D) throws CanceledException {
